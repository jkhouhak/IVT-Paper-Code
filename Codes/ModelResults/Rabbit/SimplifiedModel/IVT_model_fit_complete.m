...
for inc = 1:prec_times_len
  ...
  subplot(3,prec_times_len,inc+prec_times_len)
  xlabel('time (min)') 
  ylabel('mRNA')

  subplot(3,prec_times_len,inc+2*prec_times_len)
  xlabel('time (min)') 
  ylabel('Resources')
  for s = preincubation_idx
...
saveas(fig, sprintf('%s-IVT-fits.jpg',file_name));
...
%          x(1) = 23224.0449; x(3) = 0.0007008; x(4) =0.01574;x(6)=0.01072;
        if incubation_start
            dy(1) = dy(1) + max([0,x(1)*y(2)*y(3)]) - max([0,x(4)*y(1)]);
            dy(2) = dy(2)  - max([0,x(5)*y(2)]);
%           dy(2) = dy(2);
            dy(3) = dy(3) - max([0,x(3)*y(2)*y(3)])- max([0,x(6)*y(3)]);
        else
            y(2)=0;
            dy(1) = dy(1) + max([0,x(1)*y(2)*y(3)]) - max([0,x(4)*y(1)]);
            dy(2) = 0;
...
%       save Y
      ...
      %%%%%%%%%%%%%%%%%%% plots/figures/save %%%%%%%%%%%%%%%%%%%%% 
      subplot(3,plot_cols,plot_idx)
      hold on
      plot(Time,Y(1,:),'LineWidth',1.5);

      subplot(3,plot_cols,plot_idx+plot_cols)
      hold on
      plot(Time,Y(2,:),'LineWidth',2);

     subplot(3,plot_cols,plot_idx+2*plot_cols);
     hold on
