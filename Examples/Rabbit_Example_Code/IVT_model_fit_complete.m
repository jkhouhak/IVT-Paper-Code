...
parameter_lists = ["\alpha_P", "\alpha_M", "\alpha_R", ...
  "\delta_P", "\delta_{M_{0}}", "\delta_R","\beta", ...
  "\delta_{M_{1}}", ];
...
%%%%%%%%% IVT Model %%%%%%%%%
    function dy = model_IVT(y, x, incubation_start)
        ...
        dy(2) = 0; 
        dy(3) = 0;
        dy(4) = 0;%- max([0, 0.015*y(3)]);;

        if incubation_start

            dy(1) = dy(1) + max([0,x(1)*y(3)*y(4)]) - max([0,x(4)*y(1)]); 
            dy(2) = dy(2) - max([0,x(7)*y(2)]) - max([0,x(5)*y(2)]);
            dy(3) = dy(3) + max([0,x(7)*y(2)]) - max([0,x(8)*y(3)]);
            dy(4) = dy(4) - max([0,x(3)*y(3)*y(4)]) - 120*max([0,x(6)*y(4)]);
        else
            y(2)=0;
...
%       hold on
%       plot(Time,(Y(2,:)./(Y(2,:)+Y(3,:))),'LineWidth',2);
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
